...
| Matches list — empty/first-run state | todo | First-launch experience: no onboarding exists (missing feature). Design a welcome + permissions primer. |
| Match detail — section entrance choreography | todo | Sections pop in all at once; stagger hero ring draw-on, count-up stats. |
| Match detail — comments | done | Comments API exists (Kit) but no UI surfaces it on match detail. Missing feature. |
| Heatmap — hold-to-flip affordance | todo | Discoverability: first-view hint, spring on flip. |
| Runs carousel — page indicator & haptic ticks | todo | Verify swipe affordance is discoverable; add subtle haptics per step. |
| Fields — drawer content polish | todo | Row design, empty state, scan progress animation. |
| Fields — field detail / corner editor | todo | Corner editor usability pass; magnifier loupe? |
| Settings — visual pass | todo | Themed section icons OK to stay plain, but Save affordance + grouped styling review. |
| Paywall — visual pass | todo | Should feel premium; review layout, feature list, animation. |
| Backlog import view | todo | Progress presentation, celebratory completion. |
| Coach dashboard | todo | Timeline density, labeling flow, pitch/timeline transition. |
| Live match view + tab accessory | todo | Accessory expanded state richness; live pulse consistency. |
| Formation view | todo | Pitch rendering consistency with SoccerPitch. |
| Team chat | todo | Bubble styling, input bar, theming. |
...
| Watch settings | todo | Grouping, labels. |
| Widgets/complications | todo | Verify against palette; add live-match complication? |

## Missing/incomplete features noticed

- No onboarding/first-run permission primer (iOS).
